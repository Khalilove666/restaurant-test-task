...
import {Order, OrderProduct, Product, Table, Waiter} from "../store/modules/orders/types";
...
const waiters = ref<Array<Waiter>>([]);
...
onMounted(async () => {
    const res = await Promise.all([GetProducts(), GetTables(), GetWaiters()]);
    if (res[0].ok) products.value = res[0].data;
    if (res[1].ok) tables.value = res[1].data;
    if (res[2].ok) waiters.value = res[2].data;
...
function handleCreateOrder() {
    ...
    const waiter = waiters.value.find((w) => w.id === selectedWaiter.value);
...
<template>
    <div class="container-fluid">
        <template v-if="!currentOrderId">
            <h3 class="mt-4">Sifariş yarat</h3>
            <div class="row gy-2">
                <div class="col-12 col-sm-5">
                    <div>
                        <label for="table-select" class="form-label" :class="{'text-danger': tableError}">Select
                            table</label>
                        <select v-model="selectedTable"
                                class="form-select"
                                :class="{'border-danger': tableError}"
                                id="table-select"
                                @change="tableError = false"
                        >
                ...
                <div class="col-12 col-sm-5">
                    <label for="waiter-select" class="form-label" :class="{'text-danger': waiterError}">Select
                        waiter</label>
                    <select v-model="selectedWaiter"
                            class="form-select"
                            :class="{'border-danger': waiterError}"
                            id="waiter-select"
                            @change="waiterError = false"
                    >
                        <option v-for="waiter in waiters"
                                :key="waiter.id"
                                :value="waiter.id"
                        >{{ waiter.name }}
                        </option>
                ...
                <div class="col-12 col-sm-2 d-grid align-items-end">
                    <button @click="handleCreateOrder()" class="btn btn-primary btn-block" type="button">
                        Create
                    </button>
                </div>
            </div>
            ...
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tr v-for="(product, index) in currentProducts"
                        ...
                        <td>
                            <button @click="handleRemoveProduct(product.id)" class="btn btn-danger"><i
